...
constexpr const char *CACHE_DIRECTORY = "mooncake_local_cache/";
...
public:
    ...
    unique_ptr<FileHandle> OpenFile(const string &path, FileOpenFlags flags,
        ...
        if (FileSystem::IsRemoteFile(path) && mooncake_enable_local_cache) {
...
private:
    void FinalizeDataFile() {
        ...
        Lake::AddFile(oid, file_name, size);
    }
...
void ColumnstoreTable::Delete(ClientContext &context, vector<row_t> &row_ids) {
    ...
    for (idx_t row_ids_index = 0; row_ids_index < row_ids.size();) {
        ...
        Lake::DeleteFile(oid, data_files[file_number].file_name.c_str());
    }
    FinalizeInsert();
}
